		...
		<view class="collective">
			...
			<u-popup :show="popupInfo.show" @close="changeShow" mode='bottom'>
				<view style="height: 400rpx; margin: 30rpx;">
					<view style="font-weight: 900;margin-bottom: 40rpx;">发表你的观点吧：</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.getCommit(option.postId)
			this.pId = option.postId
		},
		methods: {
			handelLike(_item) {
				//调用接口
			...
			getCommit(_id) {
				$http('/communicate/getCommitList', {
					postId: _id
				}).then(res => {
